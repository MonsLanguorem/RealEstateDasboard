            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
              <div className="col-span-1 space-y-3 p-4 rounded-2xl shadow bg-white">
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  {useRealGeo && (<span className="text-xs ml-2 px-2 py-0.5 rounded bg-gray-100">{geoStatus==='loading'&&'загрузка…'}{geoStatus==='ready'&&'готово'}{geoStatus==='error'&&'ошибка — сетка'}</span>)}
                </div>
                  ...
                  {sa2.map(s=>(
                    <label key={s.code} className="flex items-center gap-2 text-sm">
                      <input type="checkbox" checked={selected.includes(s.code)}
                        onChange={()=>{ if(selected.includes(s.code)) setSelected(selected.filter(x=>x!==s.code)); else if(selected.length<3) setSelected([...selected,s.code]); }} />
                      {s.code}
                    </label>
                  ))}
                </div>
                ...
                {!useRealGeo || !geojson ? (
                  <div className="relative">
                    <svg viewBox="0 0 400 300" className="w-full h-[340px] rounded-xl border" onMouseMove={onCellMove}>
                      {sa2.map(s=>{const w=400/4,h=300/3,x=s.c*w,y=s.r*h;const row=bySA2Latest.find(r=>r.SA2_CODE===s.code);const val=valueForMetric(row||{});const color=colorGYR(val,minV,maxV,!higherIsBad(metric));const focus=s.code===focusSA2;return(
