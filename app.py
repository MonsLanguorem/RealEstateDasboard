...
st.set_page_config(page_title="Housing Dashboard — Sydney (SA2)", page_icon="🏠", layout="wide")

# ВАЖНО: здесь тройные кавычки ОТКРЫТЫ и НИЖЕ ЗАКРЫТЫ
components.html("""
<!doctype html>
<html>
<head>
  ...
  <style>html,body,#root{height:100%;margin:0;background:#f5f5f5}</style>
</head>
<body>
<div id="root"></div>
...
<script type="text/babel" data-presets="env,react">
  ...
  const {
    ...
    CartesianGrid, AreaChart, Area, ReferenceArea
  } = Recharts;

  // ====== Utils (без комментариев) ======
  ...
  // ====== Синтетика SA2 ======
  function useSyntheticData(){return useMemo(()=>{const rng=mulberry32(20250926);
    ...
    const rows=[]; sa2.forEach(s=>{let price=650000+rng()*950000;let rent=420+rng()*480;let income=70000+rng()*55000;
      const gp=.0018+(rng()-.5)*.0008,gr=.0012+(rng()-.5)*.0006,gi=.0009+(rng()-.5)*.0005;
      months.forEach((dt,t)=>{const seas=1+.02*Math.sin(2*Math.PI*(t%12)/12);
      ...
      (async()=>{
        ...
        for(const url of urls){
          try{ const r=await fetch(url,{mode:'cors'}); if(!r.ok) throw new Error('HTTP '+r.status);
               ...
               if(!aborted && gj && gj.features?.length){ setGeojson(gj); setGeoStatus('ready'); return; }
          }catch(e){}
        }
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
            ...
            <div className="space-y-2 max-h-48 overflow-auto border rounded p-2">
              {sa2.map(s=>(
                <label key={s.code} className="flex items-center gap-2 text-sm">
                  <input type="checkbox" checked={selected.includes(s.code)}
                    onChange={()=>{
          ...
          <div className="col-span-3 p-4 rounded-2xl shadow bg-white">
            ...
            {(!useRealGeo || !geojson) ? (
              <div className="relative">
                <svg viewBox="0 0 400 300" className="w-full h-[340px] rounded-xl border" onMouseMove={onCellMove}>
                  {sa2.map(s=>{
                    ...
                    return (
                      <g key={s.code} onClick={()=>setFocusSA2(s.code)}
                         onMouseEnter={e=>onCellEnter(e,s.code,fmtMetric(metric,val))}
                         onMouseLeave={()=>onCellLeave()} style={{cursor:'pointer'}}>
              ...
              <>
                <GeoMapSA2 geojson={geojson} metric={metric} bedrooms={bedrooms}
                           depositPct={depositPct} interest={interest} maxMonthly={maxMonthly}
                           lastMonthStr={lastMonthStr} onHover={setHover}
                           onLeave={()=>setHover(null)} onFocus={setFocusSA2}/>
                {hover && (
                  <div className="relative">
                    <div className="absolute pointer-events-none bg-white/95 border rounded-lg shadow px-2 py-1 text-xs"
                         style={{left:Math.max(0,hover.x),top:Math.max(0,hover.y)}}>
                      <div><b>{hover.code}</b></div>
                      <div>{metric}: <b>{hover.val}</b></div>
                      <div>Дата: {lastMonthStr}</div>
                    </div>
            ...
            <div className="text-xs opacity-70 mt-1">
              Метрика: <b>{metric}</b>. Фокус: <b>{focusSA2}</b> — <b>{fmtMetric(metric,focusVal)}</b>.
            </div>
          </div>
        </div>
        ...
        {segment==='buyers'
          ? <BuyerPanel {...{
              ...
              interest,setInterest,mortgageYears,setMortgageYears,S_priceAdj,S_depositTarget,
              S_income,LcapMap,LcapUser,maxMonthly,setMaxMonthly,targetMTI,setTargetMTI,mti
            }} />
          : <TenantPanel {...{
              focusSA2,bedrooms,rentAdjWeek,rentAdjMonth,rtiUser,S_rentAdjWeek,S_rentAdjMonth,
              incomeUser,setIncomeUser
            }} />
        }

        <TimeSeriesBlock {...{rows,months,startIndex,bedrooms,selected,sa2,segment}} />
  ...
  function BuyerPanel(p){
    const {priceAdj,income,setIncomeUser,savings,setSavings,depositPct,setDepositPct,
           interest,setInterest,mortgageYears,setMortgageYears,S_priceAdj,S_depositTarget,
           S_income,LcapMap,LcapUser,maxMonthly,setMaxMonthly,targetMTI,setTargetMTI,mti}=p;
      ...
      <div className="p-4 rounded-2xl shadow bg-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm mb-3">
          <label className="p-3 rounded-xl bg-gray-50 flex flex-col">Доход /год
            <input className="border rounded p-2 mt-1" type="number"
                   value={Number.isFinite(income)?income:''}
                   onChange={e=>setIncomeUser(+e.target.value||0)}/>
          </label>
          <label className="p-3 rounded-xl bg-gray-50 flex flex-col">Накопления
            <input className="border rounded p-2 mt-1" type="number"
                   value={Number.isFinite(savings)?savings:''}
                   onChange={e=>setSavings(+e.target.value||0)}/>
          ...
          {buyerMode==='budget' && (
            <label className="p-3 rounded-xl bg-gray-50 flex flex-col">Макс платёж (/мес)
              <input className="border rounded p-2 mt-1" type="number"
                     value={Number.isFinite(maxMonthly)?maxMonthly:''}
                     onChange={e=>setMaxMonthly(+e.target.value||0)}/>
        ...
        {buyerMode==='budget' && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            <div className="p-3 rounded-xl bg-gray-50"><div className="opacity-60">Доступная цена</div><div className="font-semibold">{fmtMoney(Math.round(P_affordable))}</div><div className="text-xs opacity-70">Слой Cap Gap на карте</div></div>
            <div className="p-3 rounded-xl bg-gray-50"><div className="opacity-60">Платёж</div><div className="font-semibold">{fmtMoney(Math.round(annuityMonthly(loanPrincipal,Number(interest||0)/100,Math.max(1,Number(mortgageYears||1)))))}/мес</div></div>
            <div className="p-3 rounded-xl bg-gray-50"><div className="opacity-60">MTI</div><div className="font-semibold">{(mti*100).toFixed(1)}%</div></div>
          </div>
        )}

        {buyerMode==='fixed25' && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            <div className="p-3 rounded-xl bg-gray-50"><div className="opacity-60">Платёж 25л</div><div className="font-semibold">{fmtMoney(Math.round(annuityMonthly(loanPrincipal,Number(interest||0)/100,25)))}/мес</div></div>
            <div className="p-3 rounded-xl bg-gray-50"><div className="opacity-60">MTI</div><div className="font-semibold">{((annuityMonthly(loanPrincipal,Number(interest||0)/100,25)*12/Math.max(1e-9,Number(income||0)))*100).toFixed(1)}%</div><div className="text-xs opacity-70">До 30% ок</div></div>
  ...
  function GeoMapSA2({geojson,metric,bedrooms,depositPct,interest,maxMonthly,lastMonthStr,onHover,onLeave,onFocus}){
    ...
    const coordsAll=[]; geojson.features.forEach(f=>{const g=f.geometry;if(!g)return;
      const push=c=>coordsAll.push(c);
      const walk=geom=>{const type=geom.type,cs=geom.coordinates;
    ...
    const sx=(W-2*pad)/(maxLon-minLon), sy=(H-2*pad)/(maxLat-minLat), s=Math.min(sx,sy);
    const proj=([lon,lat])=>[pad+(lon-minLon)*s, pad+(maxLat-lat)*s];
    const pathFromGeom=geom=>{const cs=geom.coordinates; let d="";
      if(geom.type==='Polygon'){ cs.forEach(r=>{ r.forEach((pt,i)=>{const[x,y]=proj(pt); d+=(i?`L${x},${y}`:`M${x},${y}`)}); d+="Z"}); }
      else if(geom.type==='MultiPolygon'){ cs.forEach(poly=>{ poly.forEach(r=>{ r.forEach((pt,i)=>{const[x,y]=proj(pt); d+=(i?`L${x},${y}`:`M${x},${y}`)}); d+="Z"}); }); }
      return d;
    };
    ...
    const vals=geojson.features.map(f=>{
      ...
      const P=price*({1:.85,2:1,3:1.25}[bedrooms]||1);
      const Rw=rentW*RENT, I=income;
      const PTI=P/I, RTI=(Rw*52)/I;
      const L_needed=P*(1-(Number(depositPct||0)/100));
      const gap=(L_needed-LcapMap)/Math.max(1e-9,P);
      const pick = metric==='Median Price'?P
                 : metric==='Median Rent'?Rw
                 : metric==='Median Income'?I
                 : metric==='PTI'?PTI
                 : metric==='Payment Cap Gap'?gap
                 : RTI;
    ...
    return (
      <div className="relative">
        <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-[520px] rounded-xl border bg-white"
             ...
             onMouseUp={endDrag} onMouseLeave={()=>{endDrag();onLeave&&onLeave();}}>
            ...
            {vals.map(v=>{
              ...
              return (
                <path key={v.code} d={d} fill={color} stroke="#fff" strokeWidth={.8}
                  onMouseEnter={e=>{const rect=e.currentTarget.ownerSVGElement.getBoundingClientRect();
                    onHover&&onHover({x:e.clientX-rect.left+8,y:e.clientY-rect.top+8,code:v.name,val:v.display})}}
  ...
  function tsWithMedian(rows,months,startIndex,bedrooms,selected,sa2){
    ...
    rows.forEach(r=>{ if(!set.has(r.date))return;
      if(seriesByCode[r.SA2_CODE]) seriesByCode[r.SA2_CODE].push({
        date:r.date, Rent:r.MedianRent_week*RENT, RentMonthly:r.MedianRent_week*RENT*52/12,
    ...
    rows.forEach(r=>{ if(!set.has(r.date))return;
      ...
      grouped.get(r.date).push({
        Rent:r.MedianRent_week*RENT, RentMonthly:r.MedianRent_week*RENT*52/12,
        Price:r.MedianPrice, PTI:r.MedianPrice/r.MedianIncome_annual,
        RTI:(r.MedianRent_week*RENT*52)/r.MedianIncome_annual
      });
    });
  ...
  function ChartBlock({title,seriesKey,ts,thresholds,colorizer}){
    ...
    const data=dates.map(d=>{
      const o={date:d,median:med.get(d)}; codes.forEach(c=>{const pt=(ts.seriesByCode[c]||[]).find(x=>x.date===d); o[c]=pt?pt[seriesKey]:undefined}); return o;
    });
    let ymin=Infinity,ymax=-Infinity;
    data.forEach(r=>{ if(Number.isFinite(r.median)){ymin=Math.min(ymin,r.median);ymax=Math.max(ymax,r.median)}
      codes.forEach(c=>{const v=r[c]; if(Number.isFinite(v)){ymin=Math.min(ymin,v);ymax=Math.max(ymax,v)}})
    });
    if(!Number.isFinite(ymin)){ymin=0;ymax=1}
    const inv = !(['Median Income'].includes(title) || seriesKey==='Income');
    return (
  ...
  function TimeSeriesBlock({rows,months,startIndex,bedrooms,selected,sa2,segment}){
    return (
      <div className="p-4 rounded-2xl shadow bg-white">
        ...
        {segment==='tenants' ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <ChartBlock title="RTI" seriesKey="RTI" ts={tsWithMedian(rows,months,startIndex,bedrooms,selected,sa2)}
              thresholds={{green:[0,.25],amber:[.25,.30],red:[.30,1]}} colorizer={(v,min,max)=>colorGYR(v,min,max,true)}/>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <ChartBlock title="PTI" seriesKey="PTI" ts={tsWithMedian(rows,months,startIndex,bedrooms,selected,sa2)}
              thresholds={{green:[0,8],amber:[8,10],red:[10,99]}} colorizer={(v,min,max)=>colorGYR(v,min,max,true)}/>
            <ChartBlock title="RTI" seriesKey="RTI" ts={tsWithMedian(rows,months,startIndex,bedrooms,selected,sa2)}
              thresholds={{green:[0,.25],amber:[.25,.30],red:[.30,1]}} colorizer={(v,min,max)=>colorGYR(v,min,max,true)}/>
          </div>
        )}
